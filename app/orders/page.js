...
export default function Orders() {
  ...
  async function loadOrders(url, token) {
    ...
    try {
      const res = await fetch('/api/query', {
        ...
        body: JSON.stringify({
          url, token,
          sql: `SELECT o.OrderId, o.OrderDate, o.Status, o.OrderType,
  ...
  async function handleDelete(o) {
    ...
    await fetch('/api/execute', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url, token, sql: 'DELETE FROM OrderItems WHERE OrderId=?', args: [o.OrderId] })
    });
    await fetch('/api/execute', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url, token, sql: 'DELETE FROM Orders WHERE OrderId=?', args: [o.OrderId] })
    });
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-[#2D3E50] text-white px-6 py-4 flex items-center justify-between">
        ...
        <button onClick={() => router.push('/order')} className="bg-green-500 hover:bg-green-600 text-white text-sm px-4 py-2 rounded-lg transition">
          + ny order
        </button>
      </div>
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-4">
        <div className="flex gap-3">
          ...
          <select
            ...
            className="border border-gray-300 rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#2D3E50]"
          >
            {statuses.map(s => (
              ...
              <tbody>
                {filtered.map((o, i) => (
                  <tr key={o.OrderId} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-4 py-3 text-gray-600">{o.OrderDate?.slice(0, 10)}</td>
